...
export default function SettingsPage() {
  ...
  const handleThemeChange = (mode: 'light' | 'dark' | 'auto') => {
    ...
    // Navbar-কে সিগন্যাল দেওয়া যাতে সাথে সাথে চেঞ্জ হয় (প্রো-লেভেল ট্রিক)
    window.dispatchEvent(new Event('themeChange'));
  };

  return (
    <div className="min-h-[80vh] bg-[#F8FAFC] dark:bg-slate-950 font-sans py-12 px-4 transition-colors duration-300">
      <div className="max-w-3xl mx-auto">
        
        {/* Page Header */}
        <div className="flex items-center mb-10">
          <div className="w-12 h-12 rounded-xl bg-orange-500 flex items-center justify-center text-white mr-4 shadow-lg shadow-orange-500/20">
        ...
        {/* Theme Settings Card (Glassmorphism & Premium UI) */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.2)] border border-slate-100 dark:border-slate-800 transition-all duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              <Sun className={`w-8 h-8 mb-3 ${themeMode === 'light' ? 'text-orange-500' : 'text-slate-400'}`} />
            ...
            >
              <Moon className={`w-8 h-8 mb-3 ${themeMode === 'dark' ? 'text-blue-500' : 'text-slate-400'}`} />
            ...
            >
              <Clock className={`w-8 h-8 mb-3 ${themeMode === 'auto' ? 'text-green-500' : 'text-slate-400'}`} />
